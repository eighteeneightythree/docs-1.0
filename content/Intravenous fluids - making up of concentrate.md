...
Reason tells us the fastest **[[Intravenous infusion rates|infusion time]]** is **three hours** because only 10mmol can be given per hour.

> $time = dose(mmol)/(K max)$  
> $time = 30mmol/(10mmol/hr)$  
> $time = 3hr$  
>   
> $volume(total) = volume(concentrate) + volume(solution)$  
> $volume(total) = 20mL +1000mL$  
> $volume(total) = 1020mL$  
>   
> $rate=volume/time$  
> $rate = 1020mL/3hr$  
...
> [!important] As it happen, NICE considers the addition of potassium to IV fluid bags dangerous practice.
